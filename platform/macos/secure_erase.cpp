...
bool open_read_write(const std::string& path, int& fd_out, std::string& error_out) {
    ...
    fd_out = ::open(raw_path.c_str(), O_RDWR | O_CLOEXEC);
    if (fd_out < 0) {
...
bool nvme_sanitize(int fd, nvme::SanitizeAction action, std::string& error_out) {
    ...
    if (cmd.result != 0) {
        error_out = "NVMe sanitize returned result " + std::to_string(cmd.result);
        return false;
    }
...
bool ata_security_erase(int fd, std::string& error_out) {
    ...
    error_out =
        "ATA secure erase ioctl not supported on this macOS device. "
        "Try NVMe sanitize for Apple Silicon NVMe drives, or use full-device wipe.";
    return false;
}

class MacSecureErase final : public ISecureErase {
public:
    bool is_supported(const std::string& device_path, std::string& reason_out) override {
        ...
        switch (bus) {
            ...
            case BusAta:
                reason_out =
                    "ATA secure erase via DKIOCISSUESECUREERASE (limited macOS support).";
                return true;
            default:
                reason_out = "Unsupported device path: " + device_path;
                return false;
        }
    }
    ...
    EraseResult execute(const std::string& device_path,
        ...
        if (bus == BusNvme) {
            ...
            if (ok) {
                result.success = true;
                result.message = "ATA secure erase issued to " + device_path;
            }
        } else {
        ...
        if (!ok && !result.success) {
            ...
            result.warnings.push_back("Run with sudo and ensure the disk is unmounted.");
        }
